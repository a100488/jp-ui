...
<script>
  ...
  export default {
    ...
    activated () {
      console.log(111)
      // this.init('view', 'edeb7281831e456f8359e3a9685eb914')
    },
    methods: {
      init (method, procInsId) {
        this.method = method
        if (method === 'add') {
          this.title = `新建票务代理`
        } else if (method === 'edit') {
        ...
        this.$nextTick(() => {
          ...
          this.inputForm.procInsId = procInsId
          ...
          if (method === 'edit' || method === 'view') { // 修改或者查看
            this.loading = true
            this.$http({
              url: `/test/onetomany/testDataMainForm/queryByProcInsId?procInsId=${this.inputForm.procInsId}`,
              method: 'get'
            }).then(({data}) => {
          ...
          if (valid) {
            ...
            this.$http({
              ...
              if (data && data.success) {
                callback(this.tableName, data.data.id)
              }
              this.loading = false
            })
          }
